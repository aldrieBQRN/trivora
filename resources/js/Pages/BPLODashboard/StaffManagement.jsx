...
import {
    Users, UserPlus, Search, ShieldCheck, CheckCircle2,
    XCircle, Clock, Edit2, Trash2, Key, Mail,
    User, Lock, X, AlertCircle, RotateCcw,
    ChevronLeft, ChevronRight,
} from 'lucide-react';
import { Modal, Button, Label, ErrorText, Input } from '@/Components/TMO';
...
export default function StaffManagement({
    ...
    // Add Form (employee_id is system-generated, never entered here)
    ...
    const paginatedUsers = users.slice((activePage - 1) * ITEMS_PER_PAGE, activePage * ITEMS_PER_PAGE);
    ...
    const handleAddSubmit = () => {
        addForm.post(route('bplo.users.store'), {
            onSuccess: () => {
                setAddModalOpen(false);
                addForm.reset();
                Swal.fire({
                    title: 'Account Created',
                    text: 'New BPLO staff member has been successfully added.',
                    icon: 'success',
                    timer: 2000,
    ...
    const handleEditSubmit = () => {
        ...
        editForm.put(route('bplo.users.update', editingUser.id), {
            onSuccess: () => {
                ...
                Swal.fire({
                    title: 'Updated',
                    text: 'BPLO staff account details updated.',
                    icon: 'success',
                    timer: 2000,
                    showConfirmButton: false
                });
    ...
    const handleToggleStatus = (user) => {
        ...
        Swal.fire({
            title: `${user.is_active ? 'Deactivate' : 'Activate'} Staff Account?`,
            text: `Are you sure you want to ${actionText} ${user.name}'s account?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: user.is_active ? '#DC2626' : '#1D2542',
            cancelButtonColor: '#64748B',
            confirmButtonText: `Yes, ${actionText}`
        }).then((result) => {
            if (result.isConfirmed) {
                router.patch(route('bplo.users.toggle-status', user.id), {}, {
                    preserveScroll: true,
                    onSuccess: () => {
                        Swal.fire({
                            title: 'Status Updated',
                            text: `Account has been ${user.is_active ? 'deactivated' : 'activated'}.`,
                            icon: 'success',
                            timer: 2000,
                            showConfirmButton: false
                        });
    ...
    const handleDeleteUser = (user) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('bplo.users.destroy', user.id), {
                    preserveScroll: true,
                    onSuccess: () => {
                        Swal.fire({
                            title: 'Deleted',
    ...
    return (
        <BPLOLayout title="Staff Management" role="BPLO Officer">
            <Head title="BPLO Staff Management | TRIVORA" />
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200/80 pb-5">
                <div>
                    <h1 className="text-2xl sm:text-[28px] font-extrabold tracking-tight text-slate-900 leading-tight">
                        BPLO Staff Management
                    </h1>
                    <p className="mt-1 text-xs sm:text-sm text-slate-500 max-w-2xl leading-relaxed">
                        Manage Business Permit &amp; Licensing Office personnel, payment verifiers, and plate issuing officers
                    </p>
                </div>
                <div className="flex items-center gap-2.5">
                    <button
                        ...
                        className="inline-flex items-center gap-2 rounded-lg bg-[#1D2542] px-3.5 py-2 text-xs font-bold text-white shadow-2xs hover:bg-[#2A3B5C] transition-colors cursor-pointer"
                    >
            ...
            {/* ══════════════════════════════════════════════════════════════
                2. PREMIUM KPI CARD DECK (TMO Staff Management style)
               ══════════════════════════════════════════════════════════════ */}
            <div className="mb-5 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
                <div className={`group rounded-2xl border border-slate-200/70 bg-white p-4 sm:p-5 ${CARD_SHADOW} transition-all hover:border-slate-300`}>
                    ...
                    <div className="mt-3">
                        ...
                        <p className="mt-0.5 text-xs font-semibold text-slate-700">Total Staff</p>
                    </div>
                    <div className="mt-3.5 flex items-center justify-between border-t border-slate-100 pt-2.5 text-[11px]">
                        ...
                        <span className="font-semibold text-slate-700">Licensing Unit</span>
                    </div>
                </div>
                ...
                <div className={`group rounded-2xl border border-slate-200/70 bg-white p-4 sm:p-5 ${CARD_SHADOW} transition-all hover:border-slate-300`}>
                    ...
                    <div className="mt-3">
                        ...
                        <p className="mt-0.5 text-xs font-semibold text-slate-700">Joined This Month</p>
                    </div>
                    <div className="mt-3.5 flex items-center justify-between border-t border-slate-100 pt-2.5 text-[11px]">
            ...
            {/* ══════════════════════════════════════════════════════════════
                4. STAFF DATA TABLE & MOBILE CARDS
               ══════════════════════════════════════════════════════════════ */}
            {users.length === 0 ? (
                <div className={`rounded-2xl border border-slate-200/70 bg-white p-12 text-center ${CARD_SHADOW}`}>
                    ...
                    <h3 className="text-sm font-bold text-slate-800">No staff accounts found</h3>
                    <p className="mt-1 text-xs text-slate-400 max-w-sm mx-auto">
                        {search
                            ? `No staff records match the query "${search}".`
                            : 'No staff personnel currently recorded under this filter status.'}
                    </p>
                </div>
            ) : (
                <>
                            ...
                            <table className="w-full border-collapse text-left text-xs">
                                <thead>
                                    <tr className="bg-slate-50/75 border-b border-slate-200/80 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                                        <th scope="col" className="py-3 pl-5 pr-3">Staff Member</th>
                                        <th scope="col" className="py-3 px-4">Position & Employee ID</th>
                        ...
                        <div className={`mt-1 flex items-center justify-between rounded-2xl border border-slate-200/70 bg-white px-4 py-3 ${CARD_SHADOW}`}>
                            ...
                            <div className="flex items-center gap-1.5">
                                <button
                                    ...
                                    disabled={activePage <= 1}
                                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                    className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-700 shadow-sm disabled:opacity-40"
                                >
                                    <ChevronLeft size={13} strokeWidth={2.5} />
                                </button>
                                <button
                ...
                <div className="flex flex-col gap-4">
                    <div>
                        ...
                        <Input
                            ...
                            placeholder="e.g. Officer Maria Clara"
                            required
                        />
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            placeholder="e.g. bplo.mclara@trivora.gov.ph"
                            required
                        />
...
/* ─────────────────────────────────────────────────────────────────────────
   SUBCOMPONENTS: StatusPill, DesktopStaffRow, MobileStaffCard
   ───────────────────────────────────────────────────────────────────────── */
...
function DesktopStaffRow({ u, user: userProp, onEdit, onToggleStatus, onDelete }) {
    const user = u || userProp;
    ...
    return (
        <tr className="group transition-colors hover:bg-slate-50/80">
            ...
            <td className="py-3.5 pl-5 pr-3 align-middle">
                <div className="flex items-center gap-2.5">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#1D2542] to-[#2A3560] text-[11px] font-bold text-white">
                        {initials}
                    </div>
            ...
            <td className="py-3.5 px-4 align-middle">
                <div className="flex flex-col gap-1">
                    <span className="inline-flex w-fit items-center gap-1.5 rounded-md bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-700">
                        ...
                        {user.position || user.role_label || 'BPLO Personnel'}
                    </span>
                    {user.employee_id ? (
...
function MobileStaffCard({ u, user: userProp, onEdit, onToggleStatus, onDelete }) {
    const user = u || userProp;
    const initials = getInitials(user.name);
        ...
        <div className={`rounded-2xl border border-slate-200/70 bg-white p-3.5 transition-all hover:border-slate-300 ${CARD_SHADOW}`}>
            ...
            <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                <div className="flex items-center gap-2 min-w-0">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#1D2542] to-[#2A3560] text-[10px] font-bold text-white">
                        {initials}
                    </div>
                    <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
